...
CURRENT_USER=$(whoami)
CPU_SYS=$(echo "$CPU_INFO" | awk -v user="$CURRENT_USER" '$2 != user {sum+=$1} END {print sum/(100.0 * '"$CORE_COUNT"')}')
CPU_USER=$(echo "$CPU_INFO" | awk -v user="$CURRENT_USER" '$2 == user {sum+=$1} END {print sum/(100.0 * '"$CORE_COUNT"')}')
