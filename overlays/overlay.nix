...
self: super: {
  ...
  opencode =
    let
      ...
      upstreamHashes =
        (builtins.fromJSON (builtins.readFile "${inputs.opencode}/nix/hashes.json")).nodeModules;
      nodeModulesHash =
        bunSources.nodeModulesHashes.${system} or upstreamHashes.${system}
          or (throw "opencode overlay: missing node_modules hash for system '${system}'");
